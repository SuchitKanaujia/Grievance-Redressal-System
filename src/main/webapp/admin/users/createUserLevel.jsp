...
String ulName = request.getParameter("user-level-name");
String ulDesc = request.getParameter("user-level-description");
...
try{
	...
	PreparedStatement ps = con.prepareStatement("insert into user_level(ul_name, ul_desc, a_name, a_dgbg, a_email, a_contact, a_paddr, a_caddr, a_fdetails, a_mdetails, a_st_basic, a_procter, a_emp_basic, a_depdesdoj,a_domdoj, a_domdesdoj, a_hostel, a_pwd, f_reg_comp, f_track_comp, f_app_comp, f_mod_comp, f_gen_rep, f_user_mng, f_site_mng, f_edit_profile) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	{
		out.print("<center><h4 style='color:green;'>User Level Created Successfully!</h4></center>");
	}
	else{
		out.print("<center><h4 style='color:red;'>Could Not Insert Data</h4></center>");
	}
	con.close();
}catch(Exception ex)
{
