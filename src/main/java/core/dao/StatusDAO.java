...
public class StatusDAO {
	public static ArrayList<Status> fetchStatus(String whereClause){
		...
		try {
			...
			ArrayList<String> selectedColumns = new ArrayList<>(Arrays.asList
					("Id", "Name", "Description", "Order", "TTL"));
			String selectQuery = (whereClause.equals("")) ? SQLQueries.generateSelectQuery("tbl_Status", selectedColumns) : 
	...
	public static boolean insertStatus(Status st){
		try {
			...
			if (st.getName() != null) toInsert.put("Name", new GenericType("'" + st.getName()+ "'"));
			if (st.getDescription() != null) toInsert.put("Description", new GenericType("'" + st.getDescription()+ "'"));
			if (st.getOrder() != 0) toInsert.put("Order", new GenericType(st.getOrder()));
			if (st.getTTL() != 0) toInsert.put("TTL", new GenericType(st.getTTL()));
	...
	public static boolean updateStatus(Status st){
		try {
			...
			if (st.getName() != null) toUpdate.put("Name", new GenericType("'" + st.getName()+ "'"));
			if (st.getDescription() != null) toUpdate.put("Description", new GenericType("'" + st.getDescription()+ "'"));
			if (st.getOrder() != 0) toUpdate.put("Order", new GenericType(st.getOrder()));
			if (st.getTTL() != 0) toUpdate.put("TTL", new GenericType(st.getTTL()));
